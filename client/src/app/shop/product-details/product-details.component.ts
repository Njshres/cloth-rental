  ...
  ngOnInit(): void {
    ...
    this.currentUserSubscription = this.currentUser$.subscribe(currentUser => {
      if (currentUser) {
        ...
        this.canUserComment(currentUser.email);
        // Do something with the displayName property here
      }
    });
  }

  canUserComment(userEmail:string){
    const id = this.activatedRoute.snapshot.paramMap.get('id');
const email = userEmail;
const url = `https://localhost:5001/api/products/canComment/${id}?email=${email}`;

this.http.get<boolean>(url).subscribe(result => {
  console.log(result);
  this.canComment = result
});
    // this.shopService.canComment(+this.activatedRoute.snapshot.paramMap.get('id'),this.userEmail).subscribe((response:boolean) => {
