...
namespace Infrastructure.Data
{
    public class GenericRepository<T> : IGenericRepository<T> where T : BaseEntity
    {
        ...
        public bool canComment(int id, string email)
        {

            var count = _context.Orders
    .Join(_context.OrderItems,
          order => order.Id,
          orderItem => orderItem.ItemOrdered.ProductItemId,
          (order, orderItem) => new { Order = order, OrderItem = orderItem })
    .Where(x => x.Order.BuyerEmail == email && x.OrderItem.ItemOrdered.ProductItemId == id)
    .Count();
